...
public class Mastermind4 {
    ...
    public int computeExacts(String guess1) {
        int exact = 0;
        for (int i = 0; i < secretlength; i++) {
            if (guess1.charAt(i) == secret.charAt(i)) {
                exact++;
            }
        }
        return exact;
    ...
    public int computePartials(String guess1) {
        int partial = 0;
        for (int i = 0; i <secretlength; i++) {
            for (int j = 0; j < secretlength; j++) {
                if (i != j && guess1.charAt(i) == secret.charAt(j)) {
                    partial++;
                }
            }
        }
    ...
    public void main(String args[]){
        ...
        for ( i=0; i < 3; i++) {
            guess = getGuess();
            exact = computeExacts(guess);
            partial = computePartials(guess);
